<div class="modal_head">
	<span><?php if (! self::$_tpl_vars['group']['id']): ?>Добавить<?php else: ?>Редактировать<?php endif; ?> партию</span>
	<i class="icon icon_delete" onclick="common.modal_hide();"></i>	
</div>
<div style="padding: 20px; border-radius: 0 0 3px 3px; background: #fff;">
	<div style="padding-top: 0px;">
		<div class="input_label">Название партии</div>
		<input class="input_primary" id="title" type="text" value="<?php echo self::$_tpl_vars['group']['title']; ?>
" placeholder="Название" autocomplete="off" autocorrect="off" autocapitalize="off">
	</div>
	<div style="padding-top: 15px;">
		<div class="input_label">Заказчик</div>
		<div style="width: 100%; position: relative;">
			<input type="text" id="customer_title" class="input_primary" placeholder="Наименование" value="<?php echo self::$_tpl_vars['group']['customer']['title']; ?>
" oninput="product.search_customer(this);" autocomplete="off" autocorrect="off" autocapitalize="off">
			<input type="hidden" id="customer_id" value="<?php echo self::$_tpl_vars['group']['customer']['id']; ?>
">
			<div class="results_container"></div>
		</div>
	</div>
	<div style="padding-top: 15px;">
		<div class="input_label">Договор</div>
		<div style="width: 100%; position: relative;">
			<input type="text" id="contract_title" class="input_primary" placeholder="Наименование" value="<?php if (self::$_tpl_vars['group']['contract']['title']): ?><?php echo self::$_tpl_vars['group']['contract']['title']; ?>
 прил. <?php echo self::$_tpl_vars['group']['contract']['annex_number']; ?>
<?php endif; ?>" oninput="product.search_contract(this);" autocomplete="off" autocorrect="off" autocapitalize="off">
			<input type="hidden" id="contract_id" value="<?php echo self::$_tpl_vars['group']['contract']['id']; ?>
">
			<div class="results_container"></div>
		</div>
	</div>
	<div style="padding-top: 15px;">
		<div class="input_label">Этикетка</div>
		<div class="selector" style="position: relative;" onclick="common.results_toggle(this, event);">
			<input type="text" class="input_primary" placeholder="Название этикетки" value="<?php echo self::$_tpl_vars['group']['label_title']; ?>
" autocomplete="off" autocorrect="off" autocapitalize="off" disabled style="cursor: pointer;">
			<input type="hidden" id="label_id" value="<?php echo self::$_tpl_vars['group']['label_id']; ?>
">
			<i class="icon icon_expand" style="position: absolute; top: 1px; right: 0px; font-size: 17px; cursor: pointer; line-height: 38px; padding: 0 14px;"></i>
			<div class="results_container">
				<?php if (isset(self::$_sections['i'])) unset(self::$_sections['i']);
self::$_sections['i']['name'] = 'i';
self::$_sections['i']['loop'] = is_array($_loop=self::$_tpl_vars['labels']) ? count($_loop) : max(0, (int)$_loop); unset($_loop);
self::$_sections['i']['show'] = true;
self::$_sections['i']['max'] = self::$_sections['i']['loop'];
self::$_sections['i']['step'] = 1;
self::$_sections['i']['start'] = self::$_sections['i']['step'] > 0 ? 0 : self::$_sections['i']['loop']-1;
if (self::$_sections['i']['show']) {
    self::$_sections['i']['total'] = self::$_sections['i']['loop'];
    if (self::$_sections['i']['total'] == 0)
        self::$_sections['i']['show'] = false;
} else
    self::$_sections['i']['total'] = 0;
if (self::$_sections['i']['show']):

            for (self::$_sections['i']['index'] = self::$_sections['i']['start'], self::$_sections['i']['iteration'] = 1;
                 self::$_sections['i']['iteration'] <= self::$_sections['i']['total'];
                 self::$_sections['i']['index'] += self::$_sections['i']['step'], self::$_sections['i']['iteration']++):
?>
				<div class="result_item" onclick="common.result_select(this, '<?php echo self::$_tpl_vars['labels'][self::$_sections['i']['index']]['title']; ?>
', <?php echo self::$_tpl_vars['labels'][self::$_sections['i']['index']]['id']; ?>
, 'full');"><?php echo self::$_tpl_vars['labels'][self::$_sections['i']['index']]['title']; ?>
</div>
				<?php endfor; endif; ?>
			</div>
		</div>
	</div>
	<div style="margin-top: 20px;">
		<div class="btn_primary" onclick="product.group_edit_update(<?php echo self::$_tpl_vars['group']['id']; ?>
);">Сохранить</div>
	</div>
</div>